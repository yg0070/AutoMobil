...
function createMap() {
	...
	getfromid("mins").innerHTML = "<i class='fa fa-eye-slash'></i>地图级别:" + map.getZoom();
}
...
function getcarinfo(carno){
	...
	$.getJSON("map.do?method=loadmapinfo",{"carno":carno,"dbname":dbna}, function(json) {
	...
	setTimeout("getcarinfo(_carno)",5000);
}
...
function careteinfor(point, carno) {
	...
	myGeo.getLocation(point,function(rs) {
		...
		$.getJSON("map.do?method=loadmapinfo",{"carno" : carsno},function(mapinfos) {
			...
			+ mapinfos[0].nowdate+ "</td> </tr><tr> <td>天气状况：</td><td id='gzweat'>"
		...
		$.ajax({url : "http://php.weather.sina.com.cn/js.php?"+ $.param({
			...
			dataType : "script",success : function(json) {
				$("#gzweat").text(status1 + " " + temperature2+ '°~' + temperature1 + '°');
			}
		});
		/*$.ajax({url : "map.do?method=getWeather&city="+cityname, dataType:"json",
			success : function(json) {
				//console.log(json);
				$("#gzweat").text(json.txt+" " +json.tmp);
