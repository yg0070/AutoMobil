...
var arforpf=[];//保存站点文字
var arforclean=[];
function startload(carno,dbna){
	$.ajaxSettings.async = false;
	$.getJSON("map.do?method=getmarksid&carno="+carno+"&dbname="+dbna,function(json){
...
/*	$.getJSON("map.do?method=loadmarkforzd&dbname="+dbna, function(json) {
		$(json).each(function(i) {
			if(json[i]!=""){
				caretezdremark(json[i]);
			}
