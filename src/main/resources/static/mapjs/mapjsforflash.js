...
//窗口切换事件
	$(function() {
		//
		$("#resultall2").hide();//开始画框取车的div隐藏事件
			...
			var keyword = "" ;
		     ...
		     renderOptions:{
			     ...
			     onSearchComplete:function showres(){
			    	 ...
			    	 cars='"'+carnos+'"';
			    	 var dbname="";
			    	 if($("#customer").select2("data")!=null){
			    		 dbname=$("#customer").select2("data").id;
			    	 }
			    	$.getJSON("map.do?method=getcarinfo",{"car":cars,"dbname":dbname},
			    	function(msg) {
			 			if(resfalg){
				    		 $("#resultall2").show(500);
							resfalg=false;
			 		   ...
			 		   var num=1;
	    			   $(msg).each(function(i){
	    				      var point=new BMap.Point(msg[i].lng, msg[i].lat);
	    				      myGeo.getLocation(point,function(rs) {
	    				    	   //addComp = rs.addressComponents;
	    				    	   //adds = addComp.province + addComp.city+ addComp.district;
	    				    	   //console.log("sss"+rs.address);
	    				    	   adds=rs.address;
	    				    	   if(adds!=""){
	    				    	      $("#restable").append("<tr style='background-color:white;color:black;cursor:pointer' onmouseover='over(this)'  onmouseout='out(this)'><td>"+(num++)+"</td><td>"+msg[i].carno+"</td><td>"+msg[i].beidouid+"</td><td>"+msg[i].gpsstate+"</td><td>"+msg[i].deptname+"</td><td>"+adds+"</td><td>"+msg[i].nowdate+"</td><td>"+msg[i].drivename+"</td><td>"+msg[i].drivenumber+"</td></tr>");
	    				    	      outString=outString+"|"+msg[i].carno+","+msg[i].beidouid+","+msg[i].gpsstate+","+msg[i].deptname+","+adds+","+msg[i].nowdate+","+msg[i].drivename+","+msg[i].drivenumber;
	    				    	   }
	    				       });
	    				      /*<a href='javascript:void(0)' onclick='getaddress(this)' style='text-decoration: none;color: blue;'>"+msg[i].lng+","+msg[i].lat+"</a>
	    				       * msg[i].lng+"."+msg[i].lat
	    				       * */
	    			   });
			 		});
	...
	function cargz(obj,dbname){
		window.open('map.do?method=getmapzz&menu_id=45&gzhidvalue='+obj+"&dbname="+dbname,'zhui1','车辆跟踪');
		//parent.addTab('车辆跟踪','map.do?method=getmapzz&menu_id=45&gzhidvalue='+obj);
	}
	
	function carhf(obj,beidouid,dbname){
		//console.log();
		console.log("回放拉拉");
		window.open('map.do?method=getmaphf&menu_id=40&hzhidvalue='+obj+'&beidouid='+beidouid+"&dbname="+dbname,'hui2','轨迹回放');
		//parent.addTab('轨迹回放','map.do?method=getmaphf&menu_id=40&hzhidvalue='+obj+'&beidouid='+beidouid);
