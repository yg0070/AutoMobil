...
!function($) {
	...
	var DateRangePicker = function(element, options, cb) {
		...
		this.parentEl = (typeof options == 'object' && options.parentEl && $(options.parentEl))
				|| $(this.parentEl);
		...
		this.container.find('.calendar').on('click', '.prev',
				$.proxy(this.clickPrev, this)).on('click', '.next',
				$.proxy(this.clickNext, this)).on('click', 'button.applyBtn_s',
				$.proxy(this.clickApply_s, this)).on('click', 'td.available',
				$.proxy(this.clickDate, this)).on('mouseenter', 'td.available',
				$.proxy(this.enterDate, this)).on('mouseleave', 'td.available',
				$.proxy(this.updateFormInputs, this)).on('change',
				'select.yearselect', $.proxy(this.updateMonthYear, this)).on(
				'change', 'select.monthselect',
				$.proxy(this.updateMonthYear, this)).on('change',
		...
		.on('click', 'button.cancelBtn', $.proxy(this.clickCancel, this)).on(
				'click',
				'.daterangepicker_start_input,.daterangepicker_end_input',
				$.proxy(this.showCalendars, this)).on('click', 'li',
				$.proxy(this.clickRange, this)).on('mouseenter', 'li',
				$.proxy(this.enterRange, this)).on('mouseleave', 'li',
	...
	DateRangePicker.prototype = {
			...
			if (this.opens == 'left') {
				this.container.css({
					...
					right : $(window).width() - this.element.offset().left
							- this.element.outerWidth() - parentOffset.left,
					left : 'auto'
				});
		...
		clickApply : function(e) {
			this.updateInputText();
			if (!this.singleDatePicker || (this.singleDatePicker && !this.timePicker))
				this.hide();
			this.element.trigger('apply', this);
		},
		clickApply_s : function(e) {
			this.updateInputText();
			this.hide();
			this.element.trigger('apply', this);
		},
		...
		updateCalendars : function() {
			...
			if (this.singleDatePicker) {
				this.container.find('.applyBtn_s').addClass(this.applyClass);
				this.container.find('.applyBtn_s').html(this.locale.applyLabel);
				this.container.find('button.applyBtn_s').removeAttr('disabled');
			} else {
				this.container.find('.applyBtn_s').hide();
			}
		...
		renderCalendar : function(calendar, selected, minDate, maxDate) {
			...
			if (this.timePicker) {
				...
				html += '</select> ';
				html += '<button class="applyBtn_s btn btn-success btn-sm pull-right" disabled="disabled"></button>';
				if (this.timePicker12Hour) {
	...
	/*var scrollFunc = function (e) {
        ...
        if (e.wheelDelta) {  //判断浏览器IE，谷歌滑轮事件
        	...
        	console.log($(".daterange").data('daterangepicker').__proto__.hide);
