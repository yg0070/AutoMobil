...
$(document).ready(function() {
	...
	nav_page_height("document ready")
	...
	$('#refresh').click(function(e) {
		$.SmartMessageBox({
			title : "<i class='fa fa-refresh' style='color:green'></i> 页面刷新",
			content : "刷新前请先保存您的内容修改。",
			buttons : '[取消][立即刷新]'
		}, function(ButtonPressed) {
			if (ButtonPressed == "立即刷新" && localStorage) {
				localStorage.clear();
				sendMessageToIframe({storageClear : true});
				location.reload();
			}
	...
	$('#logout a').click(function(e) {
		...
		// ask verification
		$.SmartMessageBox({
			title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> 退出系统 <span class='txt-color-orangeDark'><strong>" + $('#show-shortcut').text() + "</strong></span> ?",
			content : "我们期待与您的下次见面，您确定要退出系统吗？",
			buttons : '[取消][确定，下次见]'

		}, function(ButtonPressed) {
			if (ButtonPressed == "确定，下次见") {
//				$ips.load('login', 'logout', {});
	...
	$('#commonsubmit').click(function() {
		$.smallBox({
			title : "提示",
			content : "<i class='fa fa-clock-o'></i> <i>搜索功能还在开发中，敬请期待，衷心感谢您的支持! <i class=\"fa fa-heart\"></i>",
			color : "#296191",
...
function nav_page_height(log) {
	if ($("#content-frame").length <= 0) {
		...
		var mainGap = 65;//iframe高度与main高度之间的补差
		var frameHeight = $("#content-frame").height()+mainGap;
		//fix BUG #32081,改变功能页面中数据表格分页条数时，iframe高度不正确
		if(log&&typeof log=="number"){
			frameHeight = log+mainGap;
		}
		//iframe初始化时，高度固定是150，此时不作处理
		if(frameHeight==150+65){return;}
		var menuHeight = $.left_panel.find("nav").outerHeight()+107;
		var windowHeight = $(window).height() - $.navbar_height;
		var maxHeight = Math.max(frameHeight,menuHeight,windowHeight);

		if(frameHeight==150+65){return;}

		//set height
		$.left_panel.css('min-height', maxHeight + 'px');
		$.root_.css('min-height', maxHeight+$.navbar_height + 'px');
		//$('#main').css('min-height', maxHeight + 'px');

		var iframe = $("#content-frame");
		//如果iframe没有设置adjustable=“false”属性，则表示可以调整
		if(iframe.attr("adjustable")!=="false"){
			//同时设置iframe的height和min-height，兼容firefox
			var gapHeight = maxHeight-mainGap;
			iframe.height(gapHeight).css("height",gapHeight+"px").css("min-height",gapHeight+"px");
		}
	}
...
$('#main').resize(function() {
	nav_page_height("main resize");
	check_if_mobile_width();
});

$('nav').resize(function() {
	nav_page_height("nav resize");
});
...
function loadURL(url, container) {
	try {
		var parseInfo = parseSubsystemPath(url);
	} catch (e) {
		/*$ips.showError(2, [e.message]);*/
		return ;
	}
	/**
	 * fix Bug #31758，点击后退按钮后页面滚动条消失
	 * 后退操作导致body上的全屏样式没有被去除，所以每次loadURL时手动去除一次
	* **/
	$("body").removeClass("nooverflow"); 
	if (parseInfo['subsystem'] != 'gateway') {
		container.empty();
		var frame = $(container).find('#content-frame');
		if (frame.length > 1) {
			frame.remove();
		}
		if (frame.length == 0) {
		...
		if (parseInfo['url'].indexOf('?') === -1) {
			parseInfo['url'] += '?__displayrunner__=gateway';
		} else {
			parseInfo['url'] += '&__displayrunner__=gateway';
		}
		drawBreadCrumb();
		document.title = $(".breadcrumb li:last-child").text();
		frame.attr({src : parseInfo['url']});
		$('#content-frame').iFrameResize({
			enablePublicMethods     : true,
			resizedCallback : function (messageData) {
				var sHeight = $(window).scrollTop()-20;
				var ssHeight = sHeight+50;
				$("#right_main").contents().find("#divSmallBoxes").css({"top":sHeight+"px","right":"-5px"});
				$("#right_main").contents().find(".modal-dialog").css({"margin-top":ssHeight+"px"});
			}

		});
		//为了保证iframe的内容每次加载都在可视区域内，每次load新页面时都讲页面重新滚动到顶部
...
function scrollSmallBox(){
	var sHeight = $(window).scrollTop() - 100;
	if ($(window).scrollTop() < 100) {
		sHeight = 100;
	}
	var ssHeight = sHeight + 50;
	$("#right_main").contents().find("#divSmallBoxes").css({"top":sHeight+"px"});
	$("#right_main").contents().find(".modal-dialog").css({"margin-top":ssHeight+"px"});
}
$(window).scroll(function(){
	scrollSmallBox();
/*	sendMessageToIframe({scroll : $(window).scrollTop()});
*/})